...
- [Generalized birthday problem: k=8 ](https://changyuchang.name/post/generalized-birthday-problem-rcpp/#generalized-birthday-problem:-k=8)
...
This package only has two functions: `simulateC` and `gbp`. These two functions are basically the same as what I used. `simulateC` simulates and reports the *probability* given *k*, room size, and the replicate size. 

```{r}
simulateC(k=2, room=23, replicate=1e5)
```
`gbp` incorporates the `simulateC` function and find the ideal room size give *k* and *p*.

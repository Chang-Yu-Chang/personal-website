...
date: '2018-03-08'
slug: how-to-build-up-this-website
categories: []
tags:
  - fun
---

The first (meaningful) post for this new website! After searching for many blog managers, I finally found out a cool and nice way to generate a good-looking website. The whole website is built under a `R` package `blogdown`. This package is based on a site generator called Hugo. With that, there are numerous [themes]( http://themes.gohugo.io) developed by other users. Some other features would be detailedly described in tutorials, here I wanna spent more time on the problems that I encountered during making the website.
...
I like the flowchart that this tutorial uses to decribe the workflow. More pitures, more intuitive.
...
###1. Theme! Choose a nicely documented theme.
...
- git: a version control system of tracking changes in code. 

- github: an online and open-source version of git. You need an github account to use `blogdown`.

- Hugo: a site generator, like a magic move to beautify your website. Themes are built under Hugo.

- netlify: deploys static website from gitnub, and provides free domains.


###3. Some commands and steps only need to be done once.

For example, to initiate the website, you need to go through the workflow once: 
  
  1. Connect Rstudio with github (under vision control, you can see this option when creating a new R project) 
  
  2. Create a new website by the function `blogdown::new_site(sample='TRUE', theme='hugo_academic')`. You can change to a theme you like. The argument `sample` create a sample in `/theme` folder, which you can refer to in the future if you remove a feature and want it back again.
  
  3. Push code to github.
  
  4. Connect the `/public` folder in your github respository (not the local folder in your laptop) to netlify. You will update the static website in your laptop, and `blogdown` will automatically update local `/public`, then you push the updated code to github.
  
  5. Drag github `/public` to netlify, set up deploy settings, check hugo version, and deploy
  
  6. Your website becomes alive!

After you connect github with netlify, you don't need to depoly manully afterwards. When you update blogs locally, the only thing you need to do is to push the updated code (the whole folder of your website, although only github `/public` is used to deploy) to github, and wait for netlify to automatically deploy. It might take a few minutes for netlifly to deploy your website, but don't worry -- things getting simplier after the first try.
...
###5. Still haven't find the solution for Chinese post. 

I have a stong incentive to write something in Chinese. There is problem updating the website locally in Rstudio Viewer when some Chinese characters showing in the website. I still don't know why.
